...
const UserManagement = () => {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    if (!window.confirm(`Apakah Anda yakin ingin mengubah status pengguna ini menjadi ${newStatus}?`)) {
      return;
    }
    ...
    { 
      ...
      render: (row) => (
        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
          ...
          row.status === 'Blocked' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'
        }`}>
          {row.status}
        </span>
      )
