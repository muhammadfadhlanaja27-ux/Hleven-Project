...
const UserProfile = () => {
  ...
  // Kategori Status: 'unpaid' | 'paid' | 'cancelled' | 'all'
  const [historyCategory, setHistoryCategory] = useState("unpaid");
  ...
  // Helper Pengecekan Status Pembayaran
  const isBookingPaid = (status) => {
    const s = String(status || "").toLowerCase();
    return ["paid", "confirmed", "dikonfirmasi", "checked_out", "checked out", "completed", "selesai"].includes(s);
  };

  const isBookingUnpaid = (status) => {
    const s = String(status || "").toLowerCase();
    return ["pending", "unpaid", "waiting_payment"].includes(s);
  };

  const isBookingCancelled = (status) => {
    const s = String(status || "").toLowerCase();
    return ["cancelled", "dibatalkan", "cancelled_by_user", "cancelled_by_system", "refund_pending", "expired"].includes(s);
  };

  // Filter Bookings berdasarkan 3 Kategori Utama
  const filteredBookings = useMemo(() => {
    let result = [...bookings];

    if (historyCategory === "unpaid") {
      result = result.filter((b) => isBookingUnpaid(b.status));
    } else if (historyCategory === "paid") {
      result = result.filter((b) => isBookingPaid(b.status));
    } else if (historyCategory === "cancelled") {
      result = result.filter((b) => isBookingCancelled(b.status));
    }
    ...
    return result;
  }, [bookings, historyCategory, sortBy]);
  ...
  return (
    <div className="bg-[#fff8f0] text-[#1e1b16] font-body-md antialiased min-h-screen">
      <main className="w-full max-w-[1280px] mx-auto px-4 md:px-10 py-8 md:py-12 flex flex-col text-left">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {activeTab === "history" && (
              /* Booking History View with 3 Strict Categories */
              <div className="flex flex-col gap-6">
                
                {/* 3 Main Sub-Tab Category Navigation Buttons */}
                <div className="grid grid-cols-3 gap-2 p-1.5 bg-[#e8e2d9] rounded-2xl border border-[#DCCFC0]/50 shadow-xs">
                  <button
                    type="button"
                    onClick={() => setHistoryCategory("unpaid")}
                    className={`py-3 px-2 rounded-xl font-label-md text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                      historyCategory === "unpaid"
                        ? "bg-[#9B5235] text-white shadow-sm"
                        : "text-[#444842] hover:bg-[#fff8f0]/60"
                    }`}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setHistoryCategory("paid")}
                    className={`py-3 px-2 rounded-xl font-label-md text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                      historyCategory === "paid"
                  ...
                  <button
                    type="button"
                    onClick={() => setHistoryCategory("cancelled")}
                    className={`py-3 px-2 rounded-xl font-label-md text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                ...
                {/* Sort Bar */}
                <div className="flex justify-between items-center bg-[#faf3ea] px-4 py-3 rounded-xl border border-[#DCCFC0]/40">
                  <span className="font-body-md text-xs font-semibold text-[#444842]">
                    Menampilkan <strong className="text-[#778873]">{filteredBookings.length}</strong> pesanan
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="font-body-md text-xs text-[#444842]">Urutan:</span>
                    <select
                      value={sortBy}
                      onChange={(e) => setSortBy(e.target.value)}
                      className="bg-white border border-[#DCCFC0] rounded-lg px-2.5 py-1 text-xs text-[#1e1b16] font-medium focus:outline-none"
                    >
                ...
                <div className="flex flex-col gap-6">
                  {filteredBookings.length === 0 ? (
                    ...
                    filteredBookings.map((item) => {
                      const paid = isBookingPaid(item.status);
                      const unpaid = isBookingUnpaid(item.status);
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="p-6 flex-grow flex flex-col justify-between gap-4">
                            ...
                            {/* Dynamic Actions Based on Payment Validation */}
                            <div className="flex flex-wrap gap-3 justify-end pt-1">
                              ...
                              {unpaid && (
                                <>
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => navigate(`/booking/${item.hotel_id || 1}/${item.booking_rooms?.[0]?.room_type_id || 101}`)}
                                    className="px-5 py-2 rounded-xl bg-[#9B5235] text-white font-label-md text-xs font-semibold hover:bg-[#7e4028] transition-colors flex items-center gap-1 shadow-xs cursor-pointer"
                                  >
      ...
      {/* E-Ticket & Order Detail Modal (Strict QR Protection) */}
      {selectedBooking && (() => {
        const isPaid = isBookingPaid(selectedBooking.status);
        const isUnpaid = isBookingUnpaid(selectedBooking.status);
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#1e1b16]/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-[#DCCFC0]/60 max-h-[90vh] text-left relative">
              ...
              {/* Modal Header according to payment status */}
              <div className={`py-8 px-6 md:px-12 text-center relative overflow-hidden flex-shrink-0 ${
                isPaid ? "bg-[#778873] text-white" : isUnpaid ? "bg-[#9B5235] text-white" : "bg-[#ba1a1a] text-white"
              }`}>
                <span className="material-symbols-outlined text-5xl mb-2 text-white">
                  {isPaid ? "check_circle" : isUnpaid ? "pending_actions" : "cancel"}
                </span>
                <h2 className="font-headline-lg text-2xl md:text-3xl font-bold mb-1">
                  {isPaid ? "E-Ticket Valid" : isUnpaid ? "Menunggu Pembayaran" : "Pesanan Dibatalkan"}
                </h2>
                <p className="font-body-lg text-sm md:text-base opacity-90">
                  {isPaid
                    ? "Reservasi Anda telah lunas dan terkonfirmasi di sistem."
                    : isUnpaid
                    ? "Selesaikan pembayaran untuk mengaktifkan QR Code Check-in."
                    : "Pemesanan ini telah dibatalkan atau melewati batas waktu."}
                </p>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="flex flex-col items-center justify-center border-t md:border-t-0 md:border-r border-[#DCCFC0]/60 pt-6 md:pt-0 md:pr-8 text-center">
                    ...
                    {isPaid ? (
                      ...
                      <div className="flex flex-col items-center text-center p-5 bg-[#FFF0E0] rounded-2xl border border-[#9B5235]/30 w-full">
                        <span className="material-symbols-outlined text-4xl text-[#9B5235] mb-2">lock</span>
                        <p className="font-headline-sm text-sm font-bold text-[#9B5235] mb-1">
                          QR Check-in Terkunci
                        </p>
                        <p className="font-body-md text-xs text-[#444842] mb-4">
                          QR Code kamar hanya akan dibuat setelah transaksi berhasil dilunasi.
                        </p>
                        <button
                          type="button"
                          onClick={() => {
                            setSelectedBooking(null);
                            navigate(`/booking/${selectedBooking.hotel_id || 1}/${selectedBooking.booking_rooms?.[0]?.room_type_id || 101}`);
                          }}
                          className="px-6 py-2.5 bg-[#9B5235] text-white font-label-md text-xs font-semibold rounded-xl hover:bg-[#7e4028] transition-colors flex items-center justify-center gap-2 w-full cursor-pointer shadow-xs"
                        >
                          <span className="material-symbols-outlined text-base">payments</span>
                          Bayar Sekarang
                        </button>
                      ...
                      <div className="flex flex-col items-center text-center p-5 bg-[#ffdad6]/50 rounded-2xl border border-[#ba1a1a]/20 w-full">
                        <span className="material-symbols-outlined text-4xl text-[#ba1a1a] mb-2">block</span>
                        <p className="font-headline-sm text-sm font-bold text-[#ba1a1a] mb-1">
                          Tidak Berlaku
                        </p>
                        <p className="font-body-md text-xs text-[#444842]">
                          E-Tiket &amp; QR Code tidak dapat diterbitkan karena status transaksi telah dibatalkan.
                        </p>
                      </div>
                    )}
                  </div>
