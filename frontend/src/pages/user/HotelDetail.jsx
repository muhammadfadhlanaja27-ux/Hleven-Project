...
const HotelDetail = () => {
  ...
  // --- State Tambahan untuk Filter Tanggal & Tamu Dinamis ---
  const todayStr = new Date().toISOString().split("T")[0];
  const next2Days = new Date(Date.now() + 2 * 86400000).toISOString().split("T")[0];

  const [checkInDate, setCheckInDate] = useState(todayStr);
  const [checkOutDate, setCheckOutDate] = useState(next2Days);
  ...
  // Handler Pilih Kamar — Meneruskan Tanggal & Jumlah Tamu ke BookingPage
  const handleBookRoom = (e, room) => {
    e.preventDefault();
    const targetHotelId = hotel?.id || id || 1;
    const targetRoomId = room?.id || 101;
    navigate(`/booking/${targetHotelId}/${targetRoomId}?checkIn=${checkInDate}&checkOut=${checkOutDate}&adults=${adults}&children=${children}`);
  };
  ...
  return (
    <div className="bg-[#fff8f0] text-[#1e1b16] font-body-md antialiased min-h-screen">
      <main className="w-full max-w-[1280px] mx-auto px-4 md:px-10 pt-6 pb-20 text-left">
        ...
        {/* Section Detail Menginap & Tamu (DITAMBAHKAN SESUAI PERMINTAAN) */}
        <section className="mt-16 pt-10 border-t border-[#DCCFC0]/40">
          <div className="bg-white rounded-2xl p-6 border border-[#DCCFC0]/50 shadow-sm space-y-4 mb-8">
